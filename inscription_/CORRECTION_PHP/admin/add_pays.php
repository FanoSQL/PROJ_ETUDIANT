...
if ($Mode==1) {//Ajouter un pays
    ...
    //SQL
    $SQL_DOUBLON = "SELECT * FROM liste_pays WHERE Pays LIKE '%$Libelle%'";
    $res_doublon = mysqli_query($conn,$SQL_DOUBLON);
    $NbDoublon = mysqli_num_rows($res_doublon);

    if ($NbDoublon==0) {//On ajoute...
        $SQL_INSERT = "INSERT INTO liste_pays (Pays) VALUES ('$Libelle')";
        //print $SQL_INSERT;
...
<form action="add_pays.php?mode=1" method="post">
    <div class="zinput">
        <input type="text" name="sLibelle" id="" placeholder="Nouveau pays">
    </div>
...
<?php
    if ($res_pays) {
        while($ListeDesPays=mysqli_fetch_assoc($res_pays)) {
            print $ListeDesPays["Pays"]."<br>";//PS C'est mieux un include !
        }
    }
?>
